...
class Fun(commands.Cog):
        ...
        if "knock knock" in message.content.lower():
            ...
            try:
                ...
                await message.channel.send(
                    "You took too much time to reply, so the person slammed the door in your face"
                )
    ...
    async def fuck(self, ctx):
        ...
        if match:
            message = random(
                from_name=f"<@!{ctx.author.id}>",
                name=f"<@!{message[match.start():match.end()]}>",
            )
