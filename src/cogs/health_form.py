...
async def fill(ctx):
    ...
    await ctx.author.dm_channel.send("Give your username and password in the format 'username':'password' (NO spaces on either side of the colon)")
    ...
    try:
        ...
        await ctx.reply("You took too long! Please look at your DM's as that's where I'm asking for the details")
        return

    if ':' not in msg.content:
        await ctx.author.dm_channel.send("Missing a colon, please format it in the form 'username':'password'")
    else:
